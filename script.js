(function(){
  ...
  function renderPreview(){
    ...
    if (projRows.length) {
      ...
      projRows.forEach(r => {
        ...
        const linkBits = [];
        if (r.link) linkBits.push('<strong>GitHub:</strong> ' + linkifyAs(r.link, ghPath(r.link)));
  ...
  function triggerPrint(){
    if (!document.getElementById('resumePaper')) {
      alert('Add at least your name before downloading.');
      goToStep(0);
      return;
    }
    window.print();
  }
  document.getElementById('downloadTop').addEventListener('click', triggerPrint);
  document.getElementById('downloadBottom').addEventListener('click', triggerPrint);
...
//         if (r.stack) nameLine += ' <span class="r-italic">| ' + esc(r.stack) + '</span>';
//         html += '<div class="r-entry">';
//         html += '<div class="r-row">' + nameLine + '<span>' + esc(r.date) + '</span></div>';
//         if (r.link) html += '<div class="r-row sub"><span class="r-italic">' + linkify(r.link) + '</span><span></span></div>';
//         if (r.bullets) html += '<ul class="r-bullets">' + bulletsFromTextarea(r.bullets) + '</ul>';
//         html += '</div>';
//       });
//       html += '</div>';
